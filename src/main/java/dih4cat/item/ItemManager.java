...
public class ItemManager {
    ...
    public void initiateData(boolean auto, String path) {
        ...
        if (!auto) {
            ...
            f.setCurrentDirectory(
                    new File(System.getProperty("user.dir") + File.separator + "Settings" + File.separator + "Data"));
    ...
    // cream dih4cat.item
    public void createItem(int id, HashMap<String, Column> attributes, Set<String> aux, String modality,
            double duration, String organizer, String status) {
    ...
    public void createColumns(List<String> listItems) {
        ...
        for (int i = 1; i < listItems.size(); ++i) {// començam a 1 perque la 0 no ens importa
            ...
            while (j < listItems.get(i).length()) {
                if (listItems.get(i).charAt(j) == '"') {// si es una descripció o tags
                    ...
                    if (listItems.get(i).charAt(j) == '[') {// tag
                        while (listItems.get(i).charAt(j) != ']') {
                            if (listItems.get(i).charAt(j) != '[' && listItems.get(i).charAt(j) != '\'')
                        ...
                        while (!(listItems.get(i).charAt(j) == '"' && listItems.get(i).charAt(j + 1) == ','
                                && listItems.get(i).charAt(j + 2) != ' ')) {
                            if (j + 2 >= listItems.get(i).length())
                                break;
                            aux += listItems.get(i).charAt(j);
                            ++j;
                        }
                ...
                else if (listItems.get(i).charAt(j) == ',' || j == listItems.get(i).length() - 1) {// si no és
                                                                                                   // descripció o ja
                                                                                                   // acaba
                    ...
                    if (isInt(aux)) {
                        ...
                        if (cols.get(elemAct).equals("Organizer.entity")) {
                            ...
                            modality = aux;
                            if (modality.equals("Presential"))
                                ++nPresential;
                            else if (modality.equals("Online"))
                                ++nOnline;
                            else if (modality.equals("Hybrid"))
                                ++nHybrid;
    ...
    public double getNumLocations(String l) {
        if (l.equals("Presencial") || l.equals("Presential"))
            return nPresential;
        if (l.equals("Online"))
            return nOnline;
        else
            return nPresential + nOnline;
    }

    public double getNumStatus(String l) {
        if (l.equals("planned"))
            return nPlanned;
        else if (l.equals("executed"))
            return nExecuted;
        else
            return nActive;
    }
    ...
    private boolean isB(String input) {
        return input.equals("True") || input.equals("False") || input.equals("true") || input.equals("false")
                || input.equals("TRUE") || input.equals("FALSE") || input.equals("VERDADERO") || input.equals("FALSO");
    }
